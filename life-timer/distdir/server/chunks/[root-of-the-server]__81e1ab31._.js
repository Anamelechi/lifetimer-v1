...
"[project]/.next-internal/server/app/api/fact/route/actions.js [app-rsc] (server actions loader, ecmascript)": ((__turbopack_context__) => {
...
"[project]/src/app/api/fact/route.js [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    async function fallback() {
        return {
            text: `You were born around ${year}. The world has changed a lot since then.`,
            year: Number(year)
        };
    }
    ...
    function extractBulletsFromHTML(html) {
        ...
        while((m = liRegex.exec(html)) !== null){
            ...
            t = stripCitations(t).replace(/\s+/g, ' ').trim();
    ...
    function decodeHtmlEntities(str) {
        if (!str) return '';
        let s = str.replace(/&#x([0-9a-fA-F]+);/g, (_, hex)=>String.fromCodePoint(parseInt(hex, 16))).replace(/&#(\d+);/g, (_, num)=>String.fromCodePoint(parseInt(num, 10))).replace(/&nbsp;/g, ' ').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&apos;/g, "'").replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&(ndash|#8211|#x2013);/g, '–').replace(/&(mdash|#8212|#x2014);/g, '—').replace(/&(ldquo|#8220|#x201C);/g, '“').replace(/&(rdquo|#8221|#x201D);/g, '”').replace(/&(lsquo|#8216|#x2018);/g, '‘').replace(/&(rsquo|#8217|#x2019);/g, '’');
        return s;
    }
    ...
    function htmlToPlain(snippetHtml) {
        ...
        const withoutRefs = snippetHtml.replace(/<sup\b[^>]*>[\s\S]*?<\/sup>/gi, '').replace(/<sub\b[^>]*>[\s\S]*?<\/sub>/gi, '');
        const text = withoutRefs.replace(/<a[^>]*>([\s\S]*?)<\/a>/gi, '$1').replace(/<\/?(b|i|strong|em|span|small)[^>]*>/gi, '').replace(/<[^>]+>/g, '');
    ...
    function firstSentence(s) {
        ...
        let t = stripCitations(decodeHtmlEntities(s))// remove leading carets/citation markers and trailing citation-only phrases
        .replace(/^\^\s*/, '').replace(/\s*Retrieved\s.+$/i, '').replace(/\s*Archived\s.+$/i, '').replace(/\s*CS1\s+[^.]+\.?$/i, '').replace(/\s*\(link\)\s*$/i, '');
    ...
    try {
        ...
        if (html) {
            ...
            while((m = token.exec(html)) !== null){
                ...
                if (m[3]) {
                    ...
                    const raw = stripCitations(htmlToPlain(m[3])).replace(/\s+/g, ' ').trim();
        ...
        return new Response(formatFact(Number(year), null, fb.text), {
            headers: {
                'Content-Type': 'text/plain; charset=utf-8'
            }
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__81e1ab31._.js.map