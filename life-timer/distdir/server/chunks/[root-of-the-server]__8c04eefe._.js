...
"[project]/.next-internal/server/app/api/astro-note/route/actions.js [app-rsc] (server actions loader, ecmascript)": ((__turbopack_context__) => {
...
"[project]/src/app/api/astro-note/route.js [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
const RULERS = {
    ...
    Leo: {
        body: 'the Sun',
        note: 'the only sign governed by a star'
    },
    Virgo: {
        body: 'Mercury',
        note: null
    ...
    Scorpio: {
        body: 'Pluto',
        note: 'traditionally ruled by Mars'
    },
    Sagittarius: {
        body: 'Jupiter',
        note: null
    ...
    Aquarius: {
        body: 'Uranus',
        note: 'traditionally ruled by Saturn'
    },
    Pisces: {
        body: 'Neptune',
        note: 'traditionally ruled by Jupiter'
    }
};
const SIGN_TRAITS = {
    Aries: [
...
function educationalLine(sign, seed) {
    ...
    const idx = [
        ...seed || ''
    ].reduce((a, c)=>a * 29 + c.charCodeAt(0) >>> 0, 0) % options.length;
    return options[idx]();
}
...
async function wikiSummaryForSign(sign) {
    ...
    return {
        text,
        url: js.content_urls?.desktop?.page || `https://en.wikipedia.org/wiki/${encodeURIComponent(title)}`
    };
}
...
async function fetchQuotable() {
    try {
        const res = await fetchWithTimeout('https://api.quotable.io/random?maxLength=140&tags=inspirational|wisdom', {
            cache: 'no-store'
        }, 6000);
        if (!res.ok) return null;
...
async function GET(req) {
    try {
        ...
        const candidateSigns = Array.from(new Set([
            sun,
            moon,
            asc,
            ...signsParam
            ...
            if (pickDate) {
                ...
                if (bPick) {
                    ...
                    if (person) {
                        ...
                        chunks.push({
                            kind: 'sign-person',
                            text: `Famous ${chosenSign}: ${person} (born ${monthName} ${dd}).`
                        });
                ...
                if (evPick) {
                    ...
                    if (evText) {
                        ...
                        chunks.push({
                            kind: 'sign-event',
                            text: `${chosenSign} season: ${evText} (${monthName} ${dd}).`
                        });
                        if (link) sources.push(link);
                    }
                }
                ...
                if (holPick) {
                    ...
                    if (holText) {
                        ...
                        chunks.push({
                            kind: 'sign-holiday',
                            text: `${chosenSign} season: ${holText} (${monthName} ${dd}).`
                        });
                        if (link) sources.push(link);
                    }
                }
            ...
            if (mm && dd) {
                ...
                if (pick) {
                    ...
                    if (person) {
                        ...
                        chunks.push({
                            kind: 'otd',
                            text: `You share a birthday with ${person}.`,
                            person
                        });
                        if (link) sources.push(link);
                    }
        ...
        if (quote?.text) {
            ...
            if (quote.url) sources.push(quote.url);
        }
        // Build final note
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8c04eefe._.js.map